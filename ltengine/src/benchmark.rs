...
use crate::detection::{parse_detection_response, LanguageDetector};
...
pub(crate) async fn run_benchmark(args: &crate::Args, provider_manager: &Arc<ProviderManager>) {
    let dataset_path = args.dataset.as_ref().expect("Dataset path required for benchmark");
    let dataset = std::fs::read_to_string(dataset_path)
        .expect("Failed to read dataset file");
    let items: Vec<BenchmarkItem> = serde_json::from_str(&dataset)
    ...
    for (i, item) in items.iter().enumerate() {
        let gt_lang = find_ground_truth_language(&item.source_language).unwrap_or(&item.source_language);
        ...
        let llm_det = if use_llm_detect {
            ...
            match provider_manager.translate(&detect_prompt.system, &detect_prompt.user).await {
                Ok(response) => {
                    if let Some((lang, confidence)) = parse_detection_response(&response) {
                        ...
                        let fallback = crate::detection::WhatlangDetector.detect(&item.source).await;
                        Some(DetectionResult {
        ...
        let whatlang_result = crate::detection::WhatlangDetector.detect(&item.source).await;
        ...
        pb.set_target_language(
            LANGUAGES.iter().find(|l| l.code == item.target_language || l.internal_code == item.target_language)
                .map(|l| l.name)
                .unwrap_or("English"),
        );
    ...
    // Print full table
    println!("\n=== Benchmark Results ===");
    let llm_header = if use_llm_detect { "Detected (LLM)" } else { "—" };
    println!("\n{:<6} {:<20} {:<10} {:<20} {:<20} {:<8} {:<12} {:<8}",
        "#", "Source Lang (GT)", "Target", llm_header, "Detected (whatlang)", "BLEU", "Exact", "Correct");

    for r in &results {
        let llm_str = r.llm_detection.as_ref().map_or_else(
            || String::new(),
            |d| d.detected.clone(),
        );
        let whatlang_str = r.whatlang_detection.detected.clone();
        let bleu_str = format!("{:.4}", r.translation.bleu);
        let exact_str = if r.translation.exact_match { "Yes" } else { "No" };
        let correct_str = if r.whatlang_detection.correct { "Yes" } else { "No" };

        println!("{:<6} {:<20} {:<10} {:<20} {:<20} {:<8} {:<12} {:<8}",
            r.idx,
            r.source_lang,
            r.target_lang,
            llm_str,
    ...
    let whatlang_correct = results.iter().filter(|r| r.whatlang_detection.correct).count();
    ...
    let whatlang_brier: f64 = results
        ...
        .map(|r| {
            ...
            let correct = if r.whatlang_detection.correct { 1.0 } else { 0.0 };
            (conf - correct).powi(2)
        })
        .sum::<f64>()
        / total as f64;
    ...
    let llm_stats = if use_llm_detect {
        let llm_correct = results.iter().filter(|r| r.llm_detection.as_ref().map_or(false, |d| d.correct)).count();
    ...
    println!("\n=== Summary ===");
    println!("Translation: BLEU = {:.4}, Exact match rate = {:.1}% ({} / {})",
        bleu_avg, exact_rate * 100.0, exact_count, total);
    println!("Whatlang Detection: accuracy = {:.1}% ({} / {}), Brier score = {:.4}",
        whatlang_acc * 100.0, whatlang_correct, total, whatlang_brier);
    print!("{}", llm_stats);
    println!();
}